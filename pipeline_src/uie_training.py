...
from paddlenlp.trainer import PdArgumentParser, Trainer, TrainingArguments, get_last_checkpoint
...
def process(args):
    print("Processing. Arguments shown below:")
    print(args.do_balance, args.train_split_ratio, args.test_split_ratio, args.dev_split_ratio)
    data_dir = "/opt/ml/processing/input/data/"
    data_name = glob(os.path.join(data_dir, "*.json"))
    print(data_name)
    with open(data_name[0], "r", encoding="utf-8") as f:
        data = json.load(f)
        print(data)


if __name__ == "__main__":
    # Create a parser with 3 args, the first is use variables, with is a list, second is a string which is used to specific do balance data or not, third is split ration that is list as well.
    parser = PdArgumentParser((TrainModelArguments, TrainDataArguments, TrainingArguments))
    model_args, data_args, training_args = parser.parse_args_into_dataclasses()
