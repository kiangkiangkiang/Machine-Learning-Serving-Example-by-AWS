...
def read_data_by_chunk(data_path: str, max_seq_len: int = 512):
    """
        ...
        for line in f:
            ...
            while len(content) > 0:
                ...
                while len(result_list) > 0:
                    if result_list[0]["start"] > result_list[0]["end"]:
                        raise ValueError(
                            f"Error in result list. Invalid start or end location (start: {result_list[0]['start']}, "
                            f"end: {result_list[0]['end']}). Please check the data in {data_path}."
                        )
                    if result_list[0]["end"] - result_list[0]["start"] > max_content_len:
                        raise ValueError(
                            f"end - start > max_content_len, {result_list[0]['end']} - {result_list[0]['start']}"
                            f" > {max_content_len}, Please adjust label index or scale up max_seq_len"
                        )
...
def drift_offsets_mapping(offset_mapping: Tuple[Tuple[int, int]]) -> Tuple[List[List[int]], int]:
    ...
    Note: 在 tokenization 後有些字會被 tokenize 在一起，所以 index 會和原本的有所差異，因此需做調整，將 tokenize 前後的 index 對齊。
