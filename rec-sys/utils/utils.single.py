...
def get_all_inters_from_yaml(file, filters):
    ...
    def fatten_env_namespace(namespace_nests, local_envs):
        for k, v in local_envs.items():
            ...
            elif (k == "dataset" or k == "phase" or k == "runner") and isinstance(v, list):
                for i in v:
...
def create_data_loader(config, place, mode="train"):
    ...
    loader = DataLoader(dataset, batch_size=batch_size, places=place, drop_last=True)
    return loader
...
def reset_auc():
    auc_var_name = ["_generated_var_0", "_generated_var_1", "_generated_var_2", "_generated_var_3"]
