...
from utils.utils_single import create_data_loader, get_abs_model, load_dy_model_class, load_yaml
...
def main(args):
    ...
    logger.info(
        "use_gpu: {}, use_xpu: {}, use_npu: {}, use_visual: {}, train_batch_size: {}, train_data_dir: {}, epochs: {}, print_interval: {}, model_save_path: {}".format(
            use_gpu,
            use_xpu,
            use_npu,
            use_visual,
            ...
            if batch_id % print_interval == 0:
                ...
                if tensor_print_dict is not None:
                    for var_name, var in tensor_print_dict.items():
                        ...
                        if use_visual:
                            log_visual.add_scalar(tag="train/" + var_name, step=step_num, value=var.numpy())
                logger.info(
                    "epoch: {}, batch_id: {}, ".format(epoch_id, batch_id)
                    + metric_str
                    + tensor_print_str
                    + " avg_reader_cost: {:.5f} sec, avg_batch_cost: {:.5f} sec, avg_samples: {:.5f}, ips: {:.5f} ins/s".format(
